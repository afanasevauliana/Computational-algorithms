class HashTableOpenAddressing:
    ...
    def insert(self, key, value): # вставка пары ключ-значение в хеш-таблицу
        ...
        while attempt < self.size:
            ...
            # Ячейка свободна или помечена как удалённая
    ...
    def search(self, key): # поиск значения по ключу
        ...
        while attempt < self.size:
            ...
            # Пустая ячейка — ключ не существует
            if self.keys[index] is None:
                print(f"Ключ {key} не найден (проверено {attempt} позиций)")
                return None
            
            # Нашли ключ и ячейка не удалена
            if self.keys[index] == key and not self.deleted[index]:
    ...
    def display(self):
        ...
        for i in range(self.size):
            if self.keys[i] is None:
                ...
                status = "удалён"
                key_str = str(self.keys[i])
                val_str = "-"
            else:
                status = "занят"
                key_str = str(self.keys[i])
                val_str = str(self.table[i])

            print(f"{i:<8} {key_str:<12} {val_str:<20} {status}")
        print("=" * 60)
...
def create_new_table():
    """Создание новой таблицы с заданным размером"""
    while True:
        try:
            ...
            return HashTableOpenAddressing(size, c1, c2)
        except ValueError:
...
def demo_auto_fill(ht):
    test_data = [
        (15, "Яблоко"), (26, "Банан"), (37, "Вишня"), 
        (48, "Даты"), (59, "Ежевика"), (70, "Инжир"),
        (81, "Виноград"), (92, "Апельсин"), (103, "Киви")
    ]
...
def main():
    ht = HashTableOpenAddressing(size=11, c1=0, c2=1)
    print("\nХеш-таблица создана с размером 11 (простое число)")
    ...
    while True:
        ...
        if choice == '1':  # Вставка
            ...
            print(f"Формула: h(k,i) = ((k mod {ht.size}) + {ht.c1}*i + {ht.c2}*i²) mod {ht.size}")
